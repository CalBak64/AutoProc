...
def track_spawned_processes(parent_pid: int, log_path: str, child_pid_set: set, lock: Lock, interval: int = 2):
    known_pids = set()
...
def track_network_activity(target_pid: int, child_pid_set: set, lock: Lock, output_path: str, interval: float = 0.5):
    seen_connections = set()
    print(" -> Monitoring network activity in real time...")

    with open(output_path, "w", encoding="utf-8") as log:
        header = "| {:<8} | {:<25} | {:<8} | {:<8} | {:<21} | {:<21} |\n".format(
            "TYPE", "PROCESS", "PID", "PORT", "LOCAL_ADDR", "REMOTE_ADDR"
        )
        log.write(header)
        log.write("|" + "-" * 115 + "|\n")
        ...
        while psutil.pid_exists(target_pid):
            try:
                with lock:
                    current_targets = set(child_pid_set)
                current_targets.add(target_pid)
                ...
                for pid in current_targets:
                    try:
                        ...
                        for conn in proc.connections(kind="inet"):
                            ...
                            key = (pid, conn.type, laddr, raddr)

                            if key not in seen_connections:
                                seen_connections.add(key)
                                conn_type = "LISTEN" if conn.status == "LISTEN" else "CONNECT"
                                log.write("| {:<8} | {:<25} | {:<8} | {:<8} | {:<21} | {:<21} |\n".format(
                                    conn_type,
                                    proc.name(),
                                    pid,
    ...
    print(f" -> Network activity log written to: {output_path}")
...
def main():
    ...
    # Shared child PID set for network tracking

    network_log_path = os.path.join(results_dir, "network_activity.txt")

    # Start child process tracking thread
    child_thread = threading.Thread(
        target=track_spawned_processes,
        args=(pid, child_log_path, child_pids, pid_lock)
    )
    child_thread.start()

    # Start network tracking thread
    network_thread = threading.Thread(
        target=track_network_activity,
        args=(pid, child_pids, pid_lock, network_log_path)
    )
