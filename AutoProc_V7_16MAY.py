        ...
        for p in psutil.process_iter(['pid', 'name']):
            if fnmatch.fnmatch(p.info['name'].lower(), target_proc):
                ...
                try:
                    ...
                    exe_path = proc.exe()
                    proc = psutil.Process(pid)
                    globals()['TARGET_EXE_PATH'] = proc.exe()
                    globals()['TARGET_PID'] = pid
                    print(f"[+] Found match: {p.info['name']} (PID: {pid})")
                    print(f"[+] Executable path: {exe_path}")
                except (psutil.AccessDenied, psutil.NoSuchProcess):
                    print("[!] Could not retrieve executable path.")
                return pid
        time.sleep(1)
...
def track_spawned_processes(parent_pid, log_path, interval=1):
    known_pids = set()
    original_ppid = parent_pid
    has_written = False

    with open(log_path, "w", encoding="utf-8") as log:
        log.write("=== Child and Orphaned Processes ===\n\n")
        ...
        try:
            ...
            #if not has_written:
                #log.write("[!] No child or orphaned processes detected.\n")

        except Exception as e:
            log.write(f"[!] Error tracking child processes: {e}\n")

def out_children(child_log_path):
    try:
        if os.path.exists(child_log_path) and os.path.getsize(child_log_path) > 0:
            os.startfile(child_log_path)
        else:
            print("\nNo child/orphaned processes detected.")
    except Exception as e:
        print(f"[!] Error opening child process log: {e}")
...
def monitor_network_connections(pid, log_path, interval=2):
            ...
            for proc in all_procs:
                try:
                    ...
                    for conn in conns:
                        ...
                        all_rows.append((proto, str(proc.pid), local, remote, conn.status))
    ...
    col_widths = {
        "Type": max(len("Type"), max(len(row[0]) for row in all_rows)),
        "PID": max(len("PID"), max(len(row[1]) for row in all_rows)),
        "Local": max(len("Local Addr"), max(len(row[2]) for row in all_rows)),
        "Remote": max(len("Remote Addr"), max(len(row[3]) for row in all_rows)),
        "Status": max(len("Status"), max(len(row[4]) for row in all_rows)),
    }

    row_fmt = "| {0:<%d} | {1:<%d} | {2:<%d} | {3:<%d} | {4:<%d} |\n" % (
        col_widths["Type"], col_widths["PID"],
        col_widths["Local"], col_widths["Remote"], col_widths["Status"]
    )
    sep_line = "+" + "+".join(["-" * (col_widths[k] + 2) for k in col_widths]) + "+\n"

    with open(log_path, "w", encoding="utf-8") as log:
        log.write(row_fmt.format("Type", "PID", "Local Addr", "Remote Addr", "Status"))
        log.write("+" + "+".join(["-" * (w + 2) for w in [
            col_widths["Type"],
            col_widths["PID"],
            col_widths["Local"],
            col_widths["Remote"],
...
# === FILE META ===
def collect_target_metadata(path):
    global TARGET_EXE_PATH, TARGET_HASHES, TARGET_FILE_META
    ...
    try:
        ...
        with open(path, "rb") as f:
            data = f.read()
            TARGET_HASHES["MD5"] = hashlib.md5(data).hexdigest()
            TARGET_HASHES["SHA1"] = hashlib.sha1(data).hexdigest()
            TARGET_HASHES["SHA256"] = hashlib.sha256(data).hexdigest()

        # --- File metadata ---
        TARGET_FILE_META["Created"] = datetime.datetime.fromtimestamp(os.path.getctime(path)).isoformat()
        TARGET_FILE_META["Modified"] = datetime.datetime.fromtimestamp(os.path.getmtime(path)).isoformat()
        TARGET_FILE_META["Size (bytes)"] = os.path.getsize(path)
        ...
        if data:
            ...
            TARGET_FILE_META["Entropy"] = round(entropy, 4)
        else:
            TARGET_FILE_META["Entropy"] = 0.0
...
def show_results(path: str):
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("\n=== Target Process ===\n")
        f.write(f"{'Process Name':<25}: {TARGET_PROC or 'Unavailable'}\n")
        f.write(f"{'Process PID':<25}: {TARGET_PID or 'Unavailable'}\n")
        f.write(f"{'Executable Path':<25}: {TARGET_EXE_PATH or 'Unavailable'}\n")
        f.write(f"{'Hashes':<25}: ")
        if TARGET_HASHES:
            f.write(f"{'Hashes':<25}:\n")
            for name, value in TARGET_HASHES.items():
                f.write(f"{'':<25}  {name:<7}: {value}\n")

        if TARGET_FILE_META:
            f.write(f"{'File Metadata':<25}:\n")
            for key, value in TARGET_FILE_META.items():
                f.write(f"{'':<25}  {key:<15}: {value}\n")
...
def main():
    ...
    meta_thread = threading.Thread(target=collect_target_metadata, args=(TARGET_EXE_PATH,))
    meta_thread.start()
